...
def get_new_writer( out_path, csv_file, row ):
    ...
    if (r > 1 - params.p_split_table):
        ...
        f = open(os.path.join(out_path, obscured_file), 'w') 
        writer = csv.DictWriter(f, fieldnames=obscured_headers)
...
def mutate_descriptions( row ):
    ...
    for col in resolver.obscure_name(row).keys():
        if (resolver.real_name(col).endswith(synthea.desc_header)):
            new_row[col] = confuse_str(line=row[resolver.real_name(col)])
        else:
            new_row[col] = row[resolver.real_name(col)]
...
def confuse( seed, in_path, out_path, verbose ):
    ...
    for in_file in path_to_files(in_path):
        if (os.access(in_file, os.R_OK)): 
            ...
            #print(f"reader for {in_file}")
            row = dict.fromkeys(reader.readline().rstrip().split(','),'')
